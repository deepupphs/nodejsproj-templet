...
app.factory("A_dashboardFactory", function ($http, $window) {
  return {
    addUserDetails: function (data) {
      url = baseAddress + "add-membership";
      return $http.post(url, data);
    },
    getSingleDetails: function (id) {
      url = baseAddress + "get-membership" + "/" + id;
      return $http.get(url);
    },
    getAllMemberships: function () {
      url = baseAddress + "get-all-memberships";
      return $http.get(url);
    },

    updateMembershipDetails: function (data) {
      url = baseAddress + "update-membership";
      return $http.post(url, data);
    },
  };
});
...
app.controller(
  ...
  function PostController(
    ...
    $scope.getAdmin = function () {
      A_dashboardFactory.getAllMemberships()
        .success(function (data) {
          if (data.success) {
            // console.log("users", data.data.results);
            //data

            $scope.allMemberships = data.data.results;
            console.log("allMemberships", $scope.allMemberships);
          } else {
            Notification.error({
    ...
    $scope.updateMembership = function (membership) {
      ...
      A_dashboardFactory.updateMembershipDetails(membership).success(function (
        data
      ) {
        console.log("data", data);
        if (data.success == true) {

...
app.factory("A_dashboardFactory", function ($http, $window) {
  return {
    getAllMonthlyReports: function (month) {
      url = baseAddress + "agency/get-monthly-reports" + "/" + month;
      return $http.get(url);
    },
    getAllDailyReports: function (date) {
      url = baseAddress + "agency/get-daily-reports" + "/" + date;
      return $http.get(url);
    },
    getAllHostReports: function (date, hostId) {
      url =
        baseAddress + "agency/get-all-host-reports" + "/" + date + "/" + hostId;
      return $http.get(url);
    },

    // updateUserDetails: function (data) {
    //   url = baseAddress + "recharge-update";
...
app.controller(
  ...
  function PostController(
    ...
    $scope.getAdmin = function () {
      ...
      //var n = d.getMonth() + 1;
      A_dashboardFactory.getAllMonthlyReports(d)
        .success(function (data) {
          if (data.success) {
            if (data.data.results.length > 0) {
              $scope.monthlyReports = data.data.results;
            } else {
              $scope.monthlyReports = [
                {
                  date: formatDate(d),
            ...
            $scope.dailyReports = [];
            $scope.hostReports = [];
            $scope.reportMonth = new Date();
            console.log("monthlyReports", $scope.monthlyReports);
          } else {
            Notification.error({
    ...
    $scope.getMonthlyReports = function (month) {
      var d = new Date(month).toISOString();
      console.log(d);
      A_dashboardFactory.getAllMonthlyReports(d)
        .success(function (data) {
          if (data.success) {
            ...
            $scope.dailyReports = [];
            $scope.hostReports = [];
            $scope.reportMonth = new Date(month);
            if (data.data.results.length > 0) {
              $scope.monthlyReports = data.data.results;
            } else {
              $scope.monthlyReports = [
    ...
    // getAllDailyReports
    $scope.getDailyReports = function (date) {
      var d = new Date(date).toISOString();
      console.log(d);
      A_dashboardFactory.getAllDailyReports(d)
        .success(function (data) {
          if (data.success) {
            $scope.dailyReports = data.data.results;
            $scope.monthlyReports = [];
            $scope.hostReports = [];
            //$scope.reportDailyHost = new Date(date);
            console.log("dailyReports", $scope.dailyReports);
          } else {
            Notification.error({
              message: `${data.data.msg}`,
              delay: 1000,
    ...
    // getAllHostReports
    $scope.getHostReports = function (date, hostId) {
      var d = new Date(date).toISOString();
      console.log(d);
      A_dashboardFactory.getAllHostReports(d, hostId)
        .success(function (data) {
          if (data.success) {
            $scope.dailyReports = [];
            $scope.monthlyReports = [];
            $scope.hostReports = data.data.results;
            //$scope.reportMonth = new Date(date);
            $scope.reportHostId = hostId;
            console.log("dailyReports", $scope.dailyReports);
          } else {
            Notification.error({
              message: `${data.data.msg}`,
    ...
    $scope.exportMonthly = function () {
      html2canvas(document.getElementById("monthlyPdf"), {
        onrendered: function (canvas) {
          ...
          pdfMake.createPdf(docDefinition).download("monthly_report.pdf");
        },
      });
    };

    $scope.exportDaily = function () {
      html2canvas(document.getElementById("dailyPdf"), {
        onrendered: function (canvas) {
          var data = canvas.toDataURL();
          var docDefinition = {
            content: [
    ...
    $scope.exportHostee = function () {
      html2canvas(document.getElementById("hostPdf"), {
        onrendered: function (canvas) {
          var data = canvas.toDataURL();
          var docDefinition = {
            content: [
          ...
          pdfMake.createPdf(docDefinition).download("host_report.pdf");
        },
      });
    };

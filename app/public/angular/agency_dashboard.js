...
app.factory("A_dashboardFactory", function ($http, $window) {
  return {
    getChartData: function () {
      url = baseAddress + "agency/get-agency-dashboard-data";
      return $http.get(url);
    },
    makeSettlements: function (data) {
      url = baseAddress + "agency/make-and-get-all-settlements";
...
app.controller(
  ...
  function PostController(
    ...
    $scope.getAdmin = function () {
      ...
      A_dashboardFactory.makeSettlements({ date: d })
      ...
      A_dashboardFactory.getChartData()
        .success(function (data) {
          if (data.success) {
            console.log("all", data.data.results);
            $scope.totalHostCount = data.data.results.totalHostCount;
            $scope.totalEarnings = data.data.results.totalEarnings;
            $scope.totalCalls = data.data.results.totalCalls;
            $scope.totalGifts = data.data.results.totalGifts;

            //data
            $scope.allLables = data.data.results.chart.labels;
            $scope.chartData = data.data.results.chart.data;
            $scope.pieChartData = [$scope.totalCalls, $scope.totalGifts];
